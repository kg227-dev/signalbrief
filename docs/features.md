...
- `[~]` **P4-1 SQLite migration** — partial (SQLite backend, canary routing mode, `file->sqlite`/`sqlite->file` replay tooling, dual-read parity checks, and rollback guardrails exist; file-store remains default active path).
