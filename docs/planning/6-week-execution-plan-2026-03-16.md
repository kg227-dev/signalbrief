...
- [ ] P6 `Architecture` continue digest runtime decomposition until fetch, enrich, archive, cost, and delivery coordination no longer live in one orchestrator file.
  ...
  - Status (Mar 13, 2026): phase 2 complete — archive persistence, run-cost accounting, digest-lock lifecycle, and incident logging/alerting are now delegated to dedicated orchestrator seams (`digest-orchestrator-archive-runtime`, `digest-orchestrator-cost-runtime`, `digest-orchestrator-lock-runtime`, `digest-orchestrator-incident-runtime`) with contract coverage; remaining decomposition focuses on HTTP transport/bootstrap concerns still in core orchestrator.
