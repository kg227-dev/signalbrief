...
- [ ] P6 `Architecture` continue digest runtime decomposition until fetch, enrich, archive, cost, and delivery coordination no longer live in one orchestrator file.
  ...
  - Status (Mar 13, 2026): phase 1 complete — archive persistence coordination and run-cost accounting moved into dedicated orchestrator seams (`digest-orchestrator-archive-runtime`, `digest-orchestrator-cost-runtime`) with new contract coverage; remaining decomposition focuses on lock/incident/http transport concerns still in core orchestrator.
