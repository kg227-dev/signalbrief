...
const CONFIG = JSON.parse(
  fs.readFileSync(path.join(APP_ROOT, "config.json"), "utf8")
);
const EMAIL_TEMPLATE = fs.readFileSync(
  path.join(APP_ROOT, "templates/email.html"),
  "utf8"
);
const { initStore, readUser, writeUser, allUsers } = require("../runtime/store");
const { sendEmail: sendEmailViaMailer, buildOpenTrackingPixel } = require("../runtime/mailer");
const { appendEngagementEvent, buildDigestId, loadEngagementEvents } = require("../runtime/engagement-events");
const { computeDigestQualityScore } = require("../runtime/quality-score");
const { applyAutoTopicLearning } = require("../runtime/personalization");
const {
...
const { parseSourceDomain: parseSourceDomainShared } = require("../../source-domain");
