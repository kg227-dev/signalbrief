...
const START_CONFIRM_TIMEOUT_MS = 4_000;
const START_CONFIRM_POLL_MS = 120;
...
async function confirmStartedRunOwnsLock(launched, opts = {}) {
  ...
  const timeoutMs = Math.max(
    500,
    toPositiveIntOrDefault(opts.timeoutMs, getStartConfirmTimeoutMs())
  );
  const pollMs = Math.max(
    50,
    toPositiveIntOrDefault(opts.pollMs, getStartConfirmPollMs())
  );
...
async function triggerDigest(opts = {}) {
  ...
  try {
    ...
    const handshake = await confirmStartedRunOwnsLock(launched, {
      timeoutMs: opts.startConfirmMs ?? opts.queueConfirmMs,
      pollMs: opts.startConfirmPollMs ?? opts.queueConfirmPollMs,
    });
    if (!handshake.ok) {
      return {
        ok: false,
