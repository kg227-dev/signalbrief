...
function sendReengagementDay4Email(user) {
  if (!user?.email) {
    return Promise.resolve(buildMailResult({
      ok: false,
      via: "none",
      skipped: true,
      error: "missing recipient email",
...
function sendReengagementDay8Email(user) {
  if (!user?.email) {
    return Promise.resolve(buildMailResult({
      ok: false,
      via: "none",
...
function sendAutoPauseConfirmationEmail(user) {
  if (!user?.email) {
    return Promise.resolve(buildMailResult({
      ok: false,
      via: "none",
