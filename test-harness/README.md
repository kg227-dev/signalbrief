...
The test harness is a QA framework for evaluating SignalBrief digest quality across a defined set of personas, running ten scored test suites that cover topic matching, relevance ranking, analysis quality, diversity, custom topics, depth control, item count, cross-day freshness, end-to-end composite scoring, and module-level smoke checks. It supports both live API runs (Perplexity + Claude) and fully cached/offline execution, with a persistent budget ledger to cap spending across runs.
...
| `personas/persona-factory.js` | `basePersona(id, name, purpose, overrides)`: constructs a full persona object with QA token (`sha256("qa-persona:<id}")`), default preferences, and merged overrides; extracts `custom_topics` from topic list | Persona object | `node:crypto`, `personas/persona-topics` |
...
| `personas/canonical/personas-depth-defs.js` | Two depth-pair personas: depth_a (`headline_plus_oneliner`) and depth_b (`headline_plus_why`), both on AI×TECH / STRATEGY / TECHNOLOGY / POLICY×REGULATORY | Depth spec array | — |
...
| `suites/05-custom-topics.js` | Delegates to `custom-topics-runtime`; verifies custom keyword items surface in digests for personas that requested them | Suite result object | `suites/custom-topics-runtime` |
...
| `suites/custom-topics-runtime.js` | `runCustomTopicsSuite`: for each persona with custom topics, builds digest and checks keyword hit count in delivered items | Suite result object | `runtime/pipeline`, `runtime/evaluator` |
...
| `suites/module-coverage/checks-runtime-cases.js` | Four smoke checks: reengagement state normalization, `daysSince` math, `loadEngagementEvents` timestamp filtering, `normalizeIntentPayload` contract | Check results | `node:fs`, `node:os`, `node:path`, `src/jobs/reengagement-runtime`, `src/runtime/engagement/engagement-events-runtime`, `src/runtime/reply/intent-service` |
...
| `stages/dataset/live-core.js` | `buildLiveOrCachedDataset`: orchestrates fetch → selection → enrichment → summary; computes `dateKey` (ET date); calls standard and custom topic fetchers; selects and deduplicates items; enriches via Claude | Dataset result object | `config`, `stages/dataset/live-fetch`, `stages/dataset/live-selection`, `stages/dataset/live-enrichment`, `stages/dataset/live-summary`, `stages/dataset/shared` |
| `stages/dataset/live-fetch.js` | `fetchStandardTopicItems`: loops config topics calling `fetchTopicNewsCached`; `fetchCustomTopicItems`: collects unique custom keywords from personas (capped at `maxCustomFetchPerRun`) and fetches each | Per-topic item arrays | `config`, `cache`, `utils/topic-utils`, `stages/dataset/shared`, Perplexity API |
