...
function buildFallbackMeta({ stage, model = null, reason = "fallback", message = null } = {}) {
  return {
...
function heuristicAnalysisScore(item, fallbackMeta = null) {
  ...
  return {
    ...
    judged: false,
    fallback: buildFallbackMeta({
      stage: fallbackMeta?.stage || "analysis_quality_v1",
      model: fallbackMeta?.model || null,
      reason: fallbackMeta?.reason || "heuristic_fallback",
      message: fallbackMeta?.message || null,
    }),
  };
}

function normalizeJudgeAnalysisResult(result, fallbackItem, fallbackMeta = null) {
  if (!result || typeof result !== "object") {
    return heuristicAnalysisScore(fallbackItem, {
      stage: fallbackMeta?.stage || "analysis_quality_v1",
      model: fallbackMeta?.model || null,
      reason: fallbackMeta?.reason || "invalid_judge_result",
      message: fallbackMeta?.message || null,
    });
  }
  const out = {
...
function normalizeDepthJudgeResult(result, pair, fallbackMeta = null) {
  const defaultRes = {
    insight_gain: 2.5,
    ...
    judged: false,
    fallback: buildFallbackMeta({
      stage: fallbackMeta?.stage || "depth_compare_v1",
      model: fallbackMeta?.model || null,
      reason: fallbackMeta?.reason || "heuristic_fallback",
      message: fallbackMeta?.message || null,
    }),
...
async function judgeAnalysisSample(sample, deps, overrideModel = null) {
  ...
  if (noJudge) {
    return heuristicAnalysisScore(sample.item, {
      stage: "analysis_quality_v1",
      model,
      reason: "judge_disabled",
      message: "no_judge flag enabled",
    });
  }
  ...
  try {
    ...
    return normalizeJudgeAnalysisResult(judged.result, sample.item, {
      ...
      message: judged?.result == null ? "judge returned no result payload" : null,
    });
  } catch (err) {
    return heuristicAnalysisScore(sample.item, {
      stage: "analysis_quality_v1",
      model,
      reason: "judge_error",
      message: err?.message || "unknown judge failure",
    });
  }
}
...
function normalizePairwiseResult(result, fallbackMeta = null) {
  if (!result || typeof result !== "object") {
    return {
      winner: "tie",
      confidence: 0.5,
      ...
      judged: false,
      fallback: buildFallbackMeta({
        stage: fallbackMeta?.stage || "pairwise_quality_v1",
        model: fallbackMeta?.model || null,
        reason: fallbackMeta?.reason || "heuristic_fallback",
        message: fallbackMeta?.message || null,
      }),
