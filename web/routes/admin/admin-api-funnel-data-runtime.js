...
function buildSourceRegistry(auditDoc, tag) {
  ...
  const sources = configuredSources.map((s) => {
    ...
    else if (!attempted) status = "no_data";
    else if (!fetchOk) status = "error";
    else if (parsedCount === 0) status = "empty";
    else if (retainedCount === 0) status = "stale";
    else if (fetched === 0) status = "silent";
    else status = "active";
  ...
  for (const doc of validDocs) {
    ...
    for (const [tag, topicData] of Object.entries(topics)) {
      ...
      const topicFetchDiag = Array.isArray(doc?.fetch?.topic_diagnostics)
        ? doc.fetch.topic_diagnostics.find((td) => td?.tag === tag)
        : null;
